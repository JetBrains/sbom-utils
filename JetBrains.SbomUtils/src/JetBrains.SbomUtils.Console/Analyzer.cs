...
class Analyzer
{
    ...
    public int Analyze(string sbom, string rootDirectory, string[] rootPackages, string[] exemptions)
    {
        ...
        var validationResult = validator.ValidateInstallation(sbomModel, rootDirectory, rootPackages, exemptions);
    ...
    public int Analyze(string sbom, BatchVerificationParams batchVerificationParams)
    {
        ...
        foreach (var product in batchVerificationParams.Products)
        {
            ...
            try
            {
                validationResult = validator.ValidateInstallation(sbomModel, product.RootDirectory, product.RootPackages, batchVerificationParams.Ignores);
            }
            catch (Exception ex)
            {
    ...
    public void PrintValidationResult(ValidationResult validationResult)
    {
        ...
        foreach (var fileVerificationResult in validationResult.FileVerificationResults)
        {
            if (fileVerificationResult.HashVerificationFailure != null)
            {
                ...
                _logger.LogWarning("File {file} from package {package} has {count} hash mismatches: {mismatches}",
                    ...
                    string.Join(", ", fileVerificationResult.HashVerificationFailure.HashMismatches.Select(m => $"{m.FaultedChecksum.Algorithm}")));
            }
        ...
        {
            _logger.LogInformation("Validation of product successfully passed, {present} files were checked, {ignored} files were ignored",
        ...
        else
        {
            if (!validationResult.Success && !string.IsNullOrEmpty(validationResult.ErrorMessage))
                ...
                _logger.LogWarning("Validation of product failed: {missing} files are missing, {hashMismatches} files has hash mismatches, {unreferenced} files found in unreferenced packages, {total} files were checked, {passed} files passed validation, {ignored} files were ignored",
